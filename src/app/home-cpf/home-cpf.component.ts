...
@Component({
  ...
  styleUrls: ['./home-cpf.component.scss'],
})
export class HomeCPFComponent implements OnInit {
  ...
  goBack(){
    ...
    this.username=this.activatedRoute.snapshot.paramMap.get('username');
    this.router.navigate(['/espaces-cpf',this.username]);
  }
  ...
  onShowDetails(id:number){
    this.username=this.activatedRoute.snapshot.paramMap.get('username');
    this.router.navigate(['/details-projet-cpf',id,this.username]);
 }

 onShowPropose(laclasse:string){
  this.username=this.activatedRoute.snapshot.paramMap.get('username');
   laclasse = this.activatedRoute.snapshot.paramMap.get('laclasse');
   this.router.navigate(['/proposer-projet-cpf',laclasse,this.username]);
}

onShowNotifications(laclasse:string){
  this.username=this.activatedRoute.snapshot.paramMap.get('username');
  laclasse = this.activatedRoute.snapshot.paramMap.get('laclasse');
  this.router.navigate(['/notifications-cpf',laclasse,this.username]);
}
